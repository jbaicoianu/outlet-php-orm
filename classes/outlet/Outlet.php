...
class Outlet {
	...
	public function save (&$obj) {
		...
		$queryid = $this->con->generateQueryID($entityCfg->table);

		$con->beginTransaction($queryid);
	...
	public function delete ($clazz, $id) {
		...
		$res = $stmt->execute($id);
		*/
		$queryid = $this->con->generateQueryID($this->config->getEntity($clazz)->table);
		$this->con->prepareAndExecute($q, $id, $queryid);
	...
	public function select ( $clazz, $query='', $params=array()) {
		...
		*/
		...
		$queryid = "db." . $this->config->getEntity($clazz)->table . "." . md5($q);
		$rows = $this->query($q, $params, $queryid);
		foreach ($rows as $row) {
	...
	public function getEntityForRow ($entityCfg, array $row) {
		if (is_string($entityCfg)) {
			$entityCfg = $this->getConfig()->getEntity($entityCfg);
		}
	 ...
	 */
	public function query ( $query='', array $params=array(), $queryid=NULL) {
		// process the query
		...
		*/
		...
		$rows = $this->con->prepareAndExecute($q, $params, $queryid);
		return $rows;
	}
