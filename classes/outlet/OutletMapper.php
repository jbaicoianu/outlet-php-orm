...
class OutletMapper {
	...
	public function load ($cls, $pk) {
		...
		// else, populate it from the database
			...
			*/
			...
			$entityCfg->castRow($rows[0]);	
			$this->populateObject($cls, $obj, $rows[0]);
		...
		if (count($assocs)) {
			...
			foreach ($assocs as $assoc) {
				...
				if ($children->isRemoveAll()) {	
					...
					$stmt->execute($pks);
					*/
					Outlet::getInstance()->getConnection()->prepareAndExecute($q, $pks);
				}
		...
		if (count($assocs)) {
			...
			foreach ($assocs as $assoc) {
				...
				if ($children->isRemoveAll()) {
					...
					$stmt->execute(array_values($pks));
					*/
					$con->prepareAndExecute($q, array_values($pks));
				}
				...
				foreach ($new as $child) {
					...
					*/
					$queryid = $con->generateQueryID($table, current($pks));
					$con->prepareAndExecute($q, array(current($pks), $id), $queryid);
				}
	...
	public function insert (&$obj, OutletEntityConfig $entityCfg) {
		...
		$stmt->execute($values);
		*/
		$queryid = $con->generateQueryID($table);
		$con->prepareAndExecute($q, $values, $queryid);
	...
	public function update(&$obj, OutletEntityConfig $entityCfg) {
		...
		if ($mod = $this->getModifiedFields($obj, $entityCfg)) {
			...
			foreach ($entityCfg->getProperties() as $key=>$f) {
				...
				*/
				$ups[] = "  {".$cls.'.'.$key."} = :".$key;
				$qvars[$key] = self::toSqlValue( $f[1], $value ); 
			}
			$q .= implode(", \n", $ups);
			...
			foreach ($entityCfg->getProperties() as $key=>$pk) {
				...
				//$clause[] = "$pk[0] = $value";
				$clause[] = "{$pk[0]} = :where_{$pk[0]}";
				$qvars["where_".$pk[0]] = self::toSqlValue( $pk[1], $entityCfg->getProp($obj, $key));
			}
			...
			$con->exec($q);
			*/
			$table = $this->config->getEntity($cls)->table;
			$queryid = $con->generateQueryID($table);
			$con->prepareAndExecute($q, $qvars, $queryid);
		}
