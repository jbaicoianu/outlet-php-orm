...
class OutletConfig {
	...
	function __construct (array $conf) {
		...
		/* DISABLED - thefind modification to support datamanager
	...
	function getConnection () {
		if (!$this->con) {
			...
			if (!isset($conn['type'])) $conn['type'] = "pdo";

			switch ($conn['type']) {
				case 'pdo':
					if (!isset($conn['dsn'])) throw new OutletConfigException('Element [connection][dsn] not found in configuration');
					if (!isset($conn['dialect'])) throw new OutletConfigException('Element [connection][dialect] not found in configuration');
				
					$dsn = $conn['dsn'];
					$driver = substr($dsn, 0, strpos($dsn, ':'));
					
					$connobj = new PDO($conn['dsn'], @$conn['username'], @$conn['password']);
					$connobj->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
					break;
				case 'datamanager':
					$connobj = null;//DataManager::singleton();
					break;
			}
			$this->con = new OutletConnection($connobj, $driver, $conn['dialect']);
		} 
		return $this->con;
	}
